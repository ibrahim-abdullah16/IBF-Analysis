"""
IBF Hazard/Vulnerability Ablation Kappa Comparison (Fixed Version)
===================================================
...
# ============================================================
# MANUAL OPTIMIZED NO-IMPACT THRESHOLDS
# Update these values from the optimal threshold analysis.
# Only the No Impact / Low boundary is modified.
# ============================================================
...
def vulnerability_index_fixed(values):
    """
    Fixed standalone vulnerability classification.

    Vulnerability is static and therefore must not use
    lead-time-specific composite thresholds.
    """
...
def load_vulnerability_categories(project_root, cyclone):
    """
    Load standalone vulnerability categories generated by
    vulnerability_categorization.py.

    These categories are independent of:
        - lead time
        - hazard
        - exposure
    ...
    file_path = (
        ...
        / f"{cyclone.capitalize()}_Vulnerability_Categories.xlsx"
    )

    if not file_path.exists():
        raise FileNotFoundError(
            f"Standalone vulnerability file not found: {file_path}"
        )

    vul = pd.read_excel(
        file_path,
        sheet_name="Vulnerability Categories"
    )

    vul.columns = [str(c).strip() for c in vul.columns]
    ...
    missing = [
        c for c in required if c not in vul.columns
    ]
    ...
    for c in ["District", "Upazila"]:
        vul[c] = vul[c].apply(clean_admin)
...
def run_cyclone(
    ...
    print(
        f"{cyclone} - "
        "HAZARD/VULNERABILITY ABLATION KAPPA (VULNERABILITY FIXED STANDALONE)"
    )
    print("=" * 76)
    ...
    vulnerability_categories = load_vulnerability_categories(
        project_root,
        cyclone,
    )
    ...
    for lead in LEADS:
        ...
        if house["Vulnerability_Class"].isna().any():
            raise ValueError(
                "Missing standalone vulnerability categories in housing merge"
            )

        if agri["Vulnerability_Class"].isna().any():
            raise ValueError(
                "Missing standalone vulnerability categories in agriculture merge"
            )
    ...
    # --------------------------------------------------------
    # DIAGNOSTIC: vulnerability consistency check
    #
    # Vulnerability is static. The same ADM3 records and
    # Norm_Vul values must be used for all lead times.
    # --------------------------------------------------------

    base_vul = (
        merged["1dlt"]["house"]
        .sort_values(["District", "Upazila"])
        [["District", "Upazila", "Norm_Vul"]]
        .reset_index(drop=True)
    )

    for check_lead in ["2dlt", "3dlt"]:

        test_vul = (
            merged[check_lead]["house"]
            .sort_values(["District", "Upazila"])
            [["District", "Upazila", "Norm_Vul"]]
            .reset_index(drop=True)
        )

        if not base_vul.equals(test_vul):
            raise RuntimeError(
                f"Norm_Vul mismatch between 1dlt and {check_lead}"
            )

    print("Vulnerability consistency check: PASSED")
    ...
    for comp in comparisons:
        ...
        for (
            ...
            for lead in LEADS:
                ...
                if predictor_name == "Vulnerability alone":
